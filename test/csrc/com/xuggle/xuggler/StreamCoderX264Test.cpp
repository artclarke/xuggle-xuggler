...
StreamCoderX264Test :: testDecodingAndEncodingH264VideoWithBFrames()
{
  ...
  }
  {
    ...
    // These are the libx264-max.ffpreset settings from FFmpeg
    oc->setProperty("coder", "1");
    oc->setProperty("flags", "+loop");
    oc->setProperty("cmp", "+chroma");
    oc->setProperty("partitions", "+parti8x8+parti4x4+partp8x8+partp4x4+partb8x8");
    oc->setProperty("me_method", "tesa");
    oc->setProperty("subq", "10");
    oc->setProperty("me_range", "24");
    oc->setProperty("g", "250");
    oc->setProperty("keyint_min", "25");
    oc->setProperty("sc_threshold", "40");
    oc->setProperty("i_qfactor", "0.71");
    oc->setProperty("b_strategy", "2");
    oc->setProperty("qcomp", "0.6");
    oc->setProperty("qmin", "10");
    oc->setProperty("qmax", "51");
    oc->setProperty("qdiff", "4");
    oc->setProperty("bf", "3");
    oc->setProperty("refs", "16");
    oc->setProperty("directpred", "3");
    oc->setProperty("trellis", "2");
    oc->setProperty("flags2", "+wpred+mixed_refs+dct8x8-fastpskip+mbtree");
    oc->setProperty("wpredp", "2");
    retval = ic->open();
    VS_TUT_ENSURE("Could not open input coder", retval >= 0);
    retval = oc->open();
    if (retval < 0)
