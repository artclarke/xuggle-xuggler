  ...
  namespace xuggle
  {
    namespace xuggler
    {
        ...
        try
        {
          ...
          if (retval)
          {
            ...
            if (coder)
            {
              ...
              RefPointer<IRational> streamBase = stream ? stream->getTimeBase()
                  : 0;
              double base = streamBase ? streamBase->getDouble() : 0;

              codec->time_base = icodec->time_base;
              if (base && av_q2d(icodec->time_base) * icodec->ticks_per_frame
                  > base && base < 1.0 / 1000)
              {
          ...
          int32_t startingByte)
      {
        ...
        if (samples && packet && mCodecContext && mOpened && mDirection
            == DECODING && mCodec->canDecode() && getCodecType()
            == ICodec::CODEC_TYPE_AUDIO)
        {
          ...
          if (inBufSize > 0 && outBufSize > 0)
          {
            ...
            if (outBuf && inBuf)
            {
              ...
              retval = avcodec_decode_audio3(mCodecContext, outBuf,
                  &outBufSize, &pkt);
            ...
            if (retval >= 0)
            {
              ...
              int32_t bytesPerSample = (IAudioSamples::findSampleBitDepth(
                  format) / 8 * getChannels());
              ...
              if (packetTs != Global::NO_PTS)
              {
                ...
                if (timeBase->getNumerator() != 0 && timeBase->getDenominator()
                    != 0)
                {
                  ...
                  // packet
                  if (mFakeNextPts != Global::NO_PTS && (tsDelta >= -1
                      && tsDelta <= 1))
                  {
          ...
          int32_t byteOffset)
      {
        ...
        if (frame && packet && mCodecContext && mOpened && mDirection
            == DECODING && mCodec->canDecode() && getCodecType()
              ...
              if (frameFinished)
              {
                ...
                if (packetTs != Global::NO_PTS)
                {
                  if (timeBase->getNumerator() != 0)
                  {
                    ...
                    // increasing
                    if (nextPts < mFakeNextPts && packet->getPts()
                        != Global::NO_PTS)
          ...
          int32_t suggestedBufferSize)
      {
        ...
        try
        {
          ...
          if (mCodecContext && mOpened && mDirection == ENCODING && packet)
          {
            ...
            suggestedBufferSize
                = FFMAX(suggestedBufferSize, FF_MIN_BUFFER_SIZE);
            ...
            if (buf && bufLen)
            {
              ...
              if (frame)
              {
                ...
                int64_t codecTimeBasePts = thisTimeBase->rescale(
                    frame->getPts(), mFakePtsTimeBase.value(),
              ...
              if (!dropFrame)
              {
                VS_LOG_TRACE("Attempting encodeVideo(%p, %p, %d, %p)",
                    ...
                    bufLen,
                    avFrame);
                retval = avcodec_encode_video(mCodecContext, buf, bufLen,
                    avFrame);
          ...
          uint32_t startingSample)
      {
        ...
        try
        {
          ...
          int16_t *avSamples =
              (samples ? samples->getRawSamples(startingSample) : 0);
          ...
          unsigned char * frameBuffer =
              (unsigned char*) mAudioFrameBuffer->getBytes(0, frameBytes);
          if (!frameBuffer)
              ...
              >= frameBytes)
          {
            ...
            if (mCodecContext->codec->id == CODEC_ID_FLAC)
            {
              ...
              // be large enough.
              bufferSize = (64 + getAudioFrameSize() * (bytesPerSample + 1))
                  * 2;
            }
            ...
            if (buf && bufferSize)
            {
              ...
              retval = avcodec_encode_audio(mCodecContext, buf,
                  (int32_t) (bufferSize * pcmCorrection),
                  (int16_t*) frameBuffer);
              ...
              if (retval >= 0)
              {
                ...
                RefPointer<IRational> thisTimeBase = getTimeBase();
                int64_t ts;
                int64_t duration = IAudioSamples::samplesToDefaultPts(
                    frameSize, getSampleRate());
                if (!thisTimeBase)
                {
                ...
                {
                  ts = thisTimeBase->rescale(mFakeCurrPts,
                      mFakePtsTimeBase.value());
                  duration = thisTimeBase->rescale(duration,
                      mFakePtsTimeBase.value());
                }
                setPacketParameters(packet, retval, ts, thisTimeBase.value(),
                    true, duration);
      ...
      StreamCoder::setPacketParameters(Packet * packet, int32_t size,
          int64_t dts, IRational *timebase, bool keyframe, int64_t duration)
      {
      ...
      int32_t
      StreamCoder::setExtraData(com::xuggle::ferry::IBuffer* src,
          int32_t offset, int32_t numBytes, bool allocNew)
      {
        ...
        if (mCodecContext->extradata_size < numBytes
      ...
      int32_t
      StreamCoder::getExtraData(com::xuggle::ferry::IBuffer *dest,
          int32_t offset, int32_t maxBytesToCopy)
      {
        ...
        int32_t bytesToCopy =
            FFMIN(maxBytesToCopy, mCodecContext->extradata_size);
        if (bytesToCopy <= 0)
          return 0;
