...
/**
 * Allows Xuggler to read from and write to many different types of Java
 * I/O objects, plus custom {@link IURLProtocolHandler} objects.
 * 
 * <p>
 * Most of the time, IContainer hides this away from you, but in
 * case you're interested, here's the underlying class doing the
 * IO magic.
 * </p>
 * 
...
public class XugglerIO implements IURLProtocolHandlerFactory
{
  ...
  /**
   * The default protocol string that this factory uses ({@value #DEFAULT_PROTOCOL}).
   */

  public final static String DEFAULT_PROTOCOL = "xuggler";
   ...
   */
  private final static XugglerIO mFactory = new XugglerIO();
  ...
  static public XugglerIO getFactory()
  {
    return mFactory;
  }

  /**
   * Generates a unique name suitable for using in the map
   * methods for the URL parameter.
   * 
   ...
   * @param handler
   *          the handler
   * @return a string that is suitable for passing to {@link IContainer}'s open
   *         methods.
   */
  public static String map(IURLProtocolHandler handler)
  {
    return map(generateUniqueName(), handler, DEFAULT_UNMAP_URL_ON_OPEN);
  }
   ...
   * 
   * @param url
   *          the unique string to use for the mapping.
   * @param handler
   *          the handler
   * @return a string that is suitable for passing to {@link IContainer}'s open
   *         methods.
  ...
  {
    return map(generateUniqueName(), input, null, DEFAULT_UNMAP_URL_ON_OPEN,
  ...
  {
    return map(generateUniqueName(), null, output, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  ...
  {
    return map(generateUniqueName(), file, file, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  ...
  {
    return map(generateUniqueName(), channel, null, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  ...
  {
    return map(generateUniqueName(), null, channel, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  /**
   * Maps a {@link WritableByteChannel} to a URL for use by Xuggler.
  ...
  {
    return map(generateUniqueName(), channel, channel, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  ...
  {
    return map(generateUniqueName(), in, null, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
  ...
  {
    return map(generateUniqueName(), null, out, DEFAULT_UNMAP_URL_ON_OPEN,
        DEFAULT_CLOSE_STREAM_ON_CLOSE);
  }
   ...
   *          the output to use
   * @param unmapOnOpen
   *          should we remove the mapping as soon as the resulting handler is
   *          opened?
   * @param closeOnClose
   *          should we call {@link Closeable#close()} on the underlying input
   *          or output objects when {@link IURLProtocolHandler#close()} is
   *          called (if supported by the underlying object)?
   * @return Returns a URL that can be passed to Xuggler's {@link IContainer}'s
   *         open method, and will result in IO being performed on the passed in
   *         streams.
   * 
   ...
   * @param handler
   *          the handler
   * @param unmapUrlOnOpen
   *          if true, when Xuggler opens the {@link IURLProtocolHandler},
   *          {@link #unmap(String)} will be called automatically.
   * @return a string that is suitable for passing to {@link IContainer}'s open
   *         methods.
   */
  public static String map(String url, IURLProtocolHandler handler,
   ...
   * so that Xuggler calls to open the URL it will call back to the handler.
   * 
   * <p>
   * 
   ...
   * @param handler
   *          An {@link IURLProtocolHandler} for the url
   * @param unmapUrlOnOpen
