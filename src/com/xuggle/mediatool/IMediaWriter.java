...
/**
 * Encodes and writes media to containers.
 * 
 * <p>
 * 
 * An {@link IMediaWriter} is a simplified interface to the Xuggler
 ...
 * {@link #encodeVideo(int, IVideoPicture)} encode
 * media into packets and write those packets to the specified container.
 * 
 * </p>
 * <p>
 * 
 ...
 * can use {@link #encodeVideo(int, BufferedImage, long, TimeUnit)} for
 * that.
 * 
 * </p>
 * <p>
 * 
 * If you are generating audio from in Java short arrays (16-bit audio)
 * but don't have an {@link IAudioSamples} object handy, don't sweat.
 * You can use {@link #encodeAudio(int, short[], long, TimeUnit)} for
 * that.
 * 
 * </p>
 */
...
public interface IMediaWriter extends IMediaTool, IMediaPipeListener
{
   ...
   * <p>
   *
   * If true {@link IMediaWriter} will buffer media data in time
   * stamp order, and only write out data when it has at least one same
   * time or later packet from all streams.
   *
   * <p>
   *
  ...
  /**
   * Test if the {@link IMediaWriter} will forcibly interleave media data.
  ...
  /**
   * Test if this {@link IMediaWriter} can write streams
   * of this type.
   * 
  ...
  /** 
   * Add an audio stream that will later have data
   * encoded with {@link #encodeAudio(int, IAudioSamples)}.
   * <p>  The time base defaults to {@link
   ...
   *        #encodeAudio(int, IAudioSamples)} for this stream
   ...
   * <p>
   * 
   * {@link IMediaWriter} will assume that these samples are to played immediately after the
   * last set of samples, or with the earliest time stamp in the container
   * if they are the first samples.
   * 
   * </p>
   * 
   * @param streamIndex The stream index, as returned from
   ...
   * <p>
   * 
   * Callers must ensure that <code>timeStamp</code>, if 
   * <code>timeUnit</code> is non-null,
   * is always monotonically increasing or an runtime exception will be raised.
   * 
   * </p>
   * 
   * @param streamIndex The stream index, as returned from
   ...
   *        prior samples.
   */
  public abstract void encodeAudio(int streamIndex, short[] samples, long timeStamp,
      TimeUnit timeUnit);
   ...
   * is always monotonically increasing or an {@link RuntimeException} will
   * be raised.
   * 
   * </p>
   * 
   ...
   */
  public abstract void encodeVideo(int streamIndex, BufferedImage image, long timeStamp,
      TimeUnit timeUnit);
   ...
   * <p>
   * 
   * Callers should call this when they have finished encoding all audio
   * and video to ensure that any cached data necessary for encoding
   * was written.
   * 
   * </p>
   */
